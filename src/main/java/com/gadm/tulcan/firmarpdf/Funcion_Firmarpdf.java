...
//    private static final String FILE = "/home/mfernandez/Descargas/test/mozilla.pdf.p7m";
    ...
    public Boolean Invocador(String Documento, File pCertificado, String Contraseña, int pagina, int UbicacionHorizontal, int UbicacionVertical) throws KeyStoreException, Exception {
        PosicionX = Integer.toString(UbicacionHorizontal);
        PosicionY = Integer.toString(UbicacionVertical);
        Pagina = Integer.toString(pagina);
        PASSWORD = Contraseña;
        ...
        return firmarDocumento(FILE, pCertificado);
    }

    public Boolean Invocador(ParametrosEntradaFirmas pDatosFirma,
            String Documento, File pCertificado,
            String Contraseña,
            int pagina,
            int UbicacionHorizontal,
            int UbicacionVertical,
            SalidasFirmarpdf salida) throws KeyStoreException, Exception {
        PosicionX = Integer.toString(UbicacionHorizontal);
        PosicionY = Integer.toString(UbicacionVertical);
        Pagina = Integer.toString(pagina);
        PASSWORD = Contraseña;
        ...
        return firmarDocumento(pDatosFirma, FILE, pCertificado, salida);
    }
    ...
    private static Properties parametros() throws IOException {
        ...
        params.setProperty(PDFSigner.SIGNING_REASON, "Firmado digitalmente");
        ...
        params.setProperty(PDFSigner.INFO_QR, "Firmado digitalmente");
//        params.setProperty(PDFSigner.TYPE_SIG, "information2");
    ...
    private static boolean firmarDocumento(ParametrosEntradaFirmas pDatosFirma, String file, File pCertificado,
            SalidasFirmarpdf enviar) throws KeyStoreException, Exception {
        EntradasFirmarpdf entradas = new EntradasFirmarpdf();
        enviar = new SalidasFirmarpdf();
        ...
        try {
            ...
            KeyStoreProvider ksp = new FileKeyStoreProvider(pCertificado);
...
//        KeyStore keyStore = KeyStoreProviderFactory.getKeyStore(PASSWORD, "TOKEN");
        ...
        byte[] signed = null;
        File ArchivoFirmar = new File(file);
        ArchivoFirmar.getAbsolutePath();
        Signer signer = Utils.documentSigner(ArchivoFirmar);
        ...
        if (x509CertificateUtils.validarX509Certificate((X509Certificate) keyStore.getCertificate(alias))) {//validación de firmaEC
            ...
            /*signed = signer.sign(docByteArry, SignConstants.SIGN_ALGORITHM_SHA1WITHRSA, key,
                    certChain, parametros(),
                    pDatosFirma);*/
           signed = signer.sign(docByteArry, "SHA256withRSA", key,
                    certChain, parametros(),
                    pDatosFirma);
            ...
            String nombreDocumento = FileUtils.crearNombreFirmado(new File(file), FileUtils.getExtension(signed), pDatosFirma.getPsSincronico());
            java.io.FileOutputStream fos = new java.io.FileOutputStream(nombreDocumento);
            File archivoSalida = new File(nombreDocumento);
            enviar.setDocFirmado(archivoSalida.getName());
...
//Abrir documento
    ...
    private static boolean firmarDocumento(String file, File pCertificado) throws KeyStoreException, Exception {
        EntradasFirmarpdf entradas = new EntradasFirmarpdf();
...
//        KeyStore keyStore = KeyStoreProviderFactory.getKeyStore(PASSWORD, "TOKEN");
        ...
        if (x509CertificateUtils.validarX509Certificate((X509Certificate) keyStore.getCertificate(alias))) {//validación de firmaEC
            ...
            ////// Permite guardar el archivo en el equipo y luego lo abre
            String nombreDocumento = FileUtils.crearNombreFirmado(new File(file), FileUtils.getExtension(signed), null);
            java.io.FileOutputStream fos = new java.io.FileOutputStream(nombreDocumento);
